...
class MenuItemRepository:
    ...
    @staticmethod
    def soft_delete(db: Session, db_obj: MenuItem) -> MenuItem:
    ...
    @staticmethod
    def count_orders_for_item(db: Session, menu_item_id: UUID) -> int:
        return db.query(OrderItem).filter(OrderItem.menu_item_id == menu_item_id).count()


    @staticmethod
    def delete_with_relations(db: Session, db_obj: MenuItem) -> None:
        db.query(BOMItem).filter(BOMItem.menu_item_id == db_obj.id).delete(synchronize_session=False)
        db.delete(db_obj)
        db.commit()
