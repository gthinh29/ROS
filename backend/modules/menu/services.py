...
def delete_menu_item(db: Session, menu_item_id: UUID) -> dict:
    ...
    order_count = MenuItemRepository.count_orders_for_item(db, menu_item_id)

    if order_count > 0:

        

        MenuItemRepository.soft_delete(db, db_obj)
    ...
    MenuItemRepository.delete_with_relations(db, db_obj)
